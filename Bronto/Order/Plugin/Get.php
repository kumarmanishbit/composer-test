...
namespace Bronto\Order\Plugin;

use Bronto\Connector\Api\TidRepositoryInterface;
use Bronto\M2\Connector\SettingsInterface;
use Bronto\M2\Core\Cookie\ReaderInterface;
...
class Get
{
    ...
    private function attachTid(OrderRepositoryInterface $orderRepo, OrderInterface $order)
    {
        ...
        if ($tid) {
            ...
            $order->setExtensionAttributes($extensionAttributes->setBrontoTid($tid));
        }
        return $order;
    }
}
