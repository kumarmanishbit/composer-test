...
namespace Bronto\Optin\Block\Checkout;
...
class LayoutProcessor implements \Magento\Checkout\Block\Checkout\LayoutProcessorInterface
{
    ...
    const NEWSLETTER_SHIPPING_TEMPLATE = 'Bronto_Optin/shipping/newsletter';
    const NEWSLETTER_REVIEW_TEMPLATE = 'Bronto_Optin/review/newsletter';

    /**
     * @param \Bronto\M2\Optin\SettingsInterface $settings
     * @param \Bronto\M2\Optin\Checkout $checkout
     * @param \Magento\Customer\Model\Session $session
     * @param \Bronto\M2\Core\Store\ManagerInterface $storeManager
     */
    public function __construct(
        \Bronto\M2\Optin\SettingsInterface $settings,
        \Bronto\M2\Optin\Checkout $checkout,
    ...
    protected function _applyFieldChange($store, $dataPrefix, &$fields, $template)
    {
        ...
        if ($visible && isset($fields[$fieldName])) {
            $fields[$fieldName] = [
                'component' => 'Bronto_Optin/js/form/element/boolean',
                'config' => [
