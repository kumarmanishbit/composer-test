...
use Bronto\Email\Model\Trigger;
...
{
    /** @var \Bronto\M2\Email\TriggerManagerInterface */
    ...
    /** @var \Bronto\M2\Connector\MiddlewareInterface */
    ...
    protected function _generalOptions($options = [])
    {
        ...
        if (isset($options['messageExtras'])) {
            $container = new \Bronto\M2\Core\DataObject();
            $container->setFields([]);
            $container->setOptions($options);
            $this->_eventManager->dispatch('bronto_email_message_extras', [
                'container' => $container
            ]);
