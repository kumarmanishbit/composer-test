...
abstract class ExtensionAbstract implements \Bronto\M2\Connector\Discovery\ExtensionInterface, \Bronto\M2\Connector\Discovery\GroupInterface
{
    ...
    /** @var \Bronto\M2\Connector\QueueManagerInterface */
    ...
    /** @var \Bronto\M2\Core\App\EmulationInterface */
    protected $_appEmulation;

    /** @var \Bronto\M2\Core\Config\FactoryInterface */
    protected $_config;

    /** @var \Bronto\M2\Connector\ConnectorInterface */
    ...
    /**
     * @param \Bronto\M2\Core\Config\FactoryInterface $config
     * @param \Bronto\M2\Connector\ConnectorInterface $connector
    ...
    public function __construct(
        ...
        \Bronto\M2\Core\Config\FactoryInterface $config,
        ...
        if (!$disabled) {
            ...
            try {
                foreach ($this->_queueManager->getOldestEvents($registration->getConnectorKey(), $batchSize) as $event) {
                    ...
                    try {
                        if (!$errorHandlerSet) {
                            set_error_handler(['\Bronto\M2\Core\Exception\ErrorHandler', 'handleError']);
                        ...
                        if (is_array($data)) {
                            if (isset($data['data']['context']['event'])) {
                                ...
                                foreach ($data['data']['context']['event'] as $type => $context) {
                                    ...
                                    $transform = new \Bronto\DataObject([
                                        'context' => $context,
                                        $type => $data['data'][$type]
                                    ]);
                                    $this->_eventManager->dispatch("bronto_connector_queue_{$type}_transform", ['transform' => $transform]);
    ...
    {
        // bronto_connector_script_contact_historical
        $script = $observer->getScript()->getObject();
        $eventAreaName = "bronto_connector_script_{$script['data']['type']}_historical";
    ...
    {
        $script = $observer->getScript()->getObject();
        $eventAreaName = "bronto_connector_script_{$script['data']['type']}_test";
        $this->_eventManager->dispatch($eventAreaName, [
            'script' => $observer->getScript()
        ]);
    }
