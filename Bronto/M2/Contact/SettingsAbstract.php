...
abstract class SettingsAbstract extends \Bronto\M2\Core\Config\ContainerAbstract implements SettingsInterface
{
    const EVENT_PREFIX = "bronto_contact_extra_";
        ...
        foreach ($this->getAttributesToModel($type, $storeId) as $key => $func) {
           // $attributes = $this->_savedAttributes($key, $store);
            ...
            if (!empty($attributes)) {
                ...
                // EVENT_PREFIX:- bronto_contact_extra_
     ...
     * @param array $attributes
     * @return \Bronto\M2\Core\DataObject
     */
    public function orderModelLoader($order, $attributes)
    {
        ...
        return new \Bronto\M2\Core\DataObject($data + ['resource' => false]);
    }
    ...
    {
        $path = 'bronto/contact/extensions/' . $attrkey;
