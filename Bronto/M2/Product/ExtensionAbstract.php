...
abstract class ExtensionAbstract extends \Bronto\M2\Connector\Discovery\AdvancedExtensionAbstract implements \Bronto\M2\Connector\Discovery\GroupInterface, CatalogMapperInterface, \Bronto\M2\Connector\Discovery\TransformEventInterface
{
    ...
    public function pushChangesToAll(DataObject $event)
    {
        ...
        $siteHash = [];
        /** @var \Bronto\Connector\Model\Registration $registration */
        foreach ($this->_registrations->getAll() as $registration) {
            ...
            if (!is_null($originalSku) && $product->dataHasChangedFor('sku')) {
                $archivalProduct = new \Bronto\M2\Core\DataObject(
                    [
                    'id' => $product->getId(),
                ...
                $archivalObserver = new \Bronto\M2\Core\DataObject(
                    [
