...
{
    const BASE_URL_OVERRIDE_KEY = 'BRONTO_MIDDLEWARE_BASEURL';
    ...
    /** @var \Bronto\Transfer\Adapter  */
    protected $_client;

    /** @var \Bronto\Serialize\BiDirectional  */
    protected $_encoder;

    /** @var \Bronto\M2\Core\Log\LoggerInterface  */
    protected $_logger;

    /** @var \Bronto\M2\Core\MetaInterface  */
    protected $_meta;

    /** @var \Bronto\M2\Core\EncryptorInterface  */
    protected $_encrypt;
    ...
    /** @var \Bronto\M2\Core\Config\ManagerInterface  */
    protected $_config;
     ...
     * @param \Bronto\M2\Core\Config\ManagerInterface $config
     * @param SettingsInterface $settings
     */
    public function __construct(
        \Bronto\Transfer\Adapter $client,
        \Bronto\Serialize\BiDirectional $encoder,
        \Bronto\M2\Core\Log\LoggerInterface $logger,
        \Bronto\M2\Core\MetaInterface $meta,
        ...
        \Bronto\M2\Core\Config\ManagerInterface $config,
    ...
    public function triggerFlush(RegistrationInterface $model)
    {
        try {
            ...
            if ($canFlush && $model->getIsActive()) {
                ...
                $script = new \Bronto\M2\Connector\Discovery\Script($model);
                $this->_eventManager->dispatch('bronto_connector_trigger_flush', [
                    'script' => $script
                ]);
    ...
    {
        $endpoint = new \Bronto\M2\Connector\Discovery\Endpoint();
        $eventAreaName = "bronto_connector_{$serviceName}_endpoint";
    ...
    {
        $source = new \Bronto\M2\Connector\Discovery\Source($model);
        // $sourceId can be like coupon_code
        $eventAreaName = "bronto_connector_source_$sourceId";
    ...
    public function settings(RegistrationInterface $model)
    {
        ...
        $response = $request->respond();
        if ($response->code() >= 300) {
            throw new \Bronto\Transfer\Exception("Failed to retrieve settings.", $response->code(), $request);
        }
    ...
    public function getEik(RegistrationInterface $model)
    {
        ...
        $response = $request->respond();
        if ($response->code() >= 300) {
            throw new \Bronto\Transfer\Exception("Failed to retrieve EIK.", $response->code(), $request);
        }
    ...
    {
        $execution = new \Bronto\M2\Connector\Discovery\Execution($model);
        $objectId = "{$object['extensionId']}_{$object["id"]}";
        $this->_logger->debug($objectId);
        $this->_eventManager->dispatch("bronto_connector_{$name}_{$objectId}", [
    ...
    {
        $discovery = new \Bronto\M2\Connector\Discovery();
        $this->_eventManager->dispatch("bronto_connector_gather_endpoints", [
            'discovery' => $discovery,
            'registration' => $model
        ]);
    ...
    {
        $defaultStore = $this->_defaultStore($model);
        $brontoCustomUrl = $this->_settings->getCustomUrl();
        if ($brontoCustomUrl) {
            $baseUrl = trim($brontoCustomUrl, '/') . '/';
        } else {
    ...
    protected function _walkSettings($model, $settings, $callback)
    {
        ...
        foreach ($settings as $general => $value) {
            $path = "bronto/general/settings/{$general}";
            if ($general == 'featureMap') {
                foreach ($value as $feature => $flag) {
                    if (preg_match('/^magento_(.+)/', $feature, $matches)) {
                        $path = "bronto/toggle/{$matches[1]}";
                    } else {
                        $path = "bronto/general/features/{$feature}";
                    }
        ...
        foreach ($scopedSettings as $scope) {
            ...
            foreach ($scope['groups'] as $endpoint) {
                ...
                $eventAfter = "bronto_{$endpoint['id']}_setting_sync_after";
    ...
    protected function _sectionWalk($section, $scopeName, $scopeId, $endpoint, $callback)
    {
        foreach ($endpoint[$section] as $object) {
            foreach ($object['fields'] as $field) {
                ...
                $path = "bronto/{$endpoint['id']}/{$section}/{$object['id']}/{$field['id']}";
    ...
    protected function _recursiveDelete($tree, $suffix = '')
    {
        ...
        $this->_config->deleteAll('bronto/' . $suffix, $scopeName, $scopeId);
