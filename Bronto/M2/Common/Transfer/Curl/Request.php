 ...
 */
class Request implements \Bronto\M2\Common\Transfer\Request
{
    private $_method;
    private $_uri;
    private $_options;
     ...
     * @param string $method
     * @param string $uri
     * @param \Bronto\M2\Common\DataObject $options
     * @param \Bronto\M2\Common\StandardResource\Proxy $curl
     */
    public function __construct($method, $uri, $options, $curl = null)
    {
        $this->_uri = $uri;
        $this->_method = $method;
        $this->_options = new \Bronto\M2\Common\DataObject($options->toArray());
        if (is_null($curl)) {
            $curl = new \Bronto\M2\Common\StandardResource\Proxy("curl_");
        }
        $this->_curl = $curl;
    }
     ...
     *
     * @return \Bronto\M2\Common\DataObject
     */
     ...
     *
     * @return \Bronto\M2\Common\StandardResource\Proxy
     */
    ...
    protected function _completeRequest()
    {
        ...
        if ($code) {
            ...
            $this->_curl->close();
            throw $this->error(new \Bronto\M2\Common\Transfer\Exception($message, $code, $this));
        }
