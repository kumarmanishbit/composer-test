...
namespace Bronto\M2\Order\Event;

use Bronto\Connector\Api\Data\TidInterface;
use Bronto\M2\Connector\Event\ContextProviderInterface;
use Bronto\M2\Connector\Event\SourceInterface;
use Bronto\M2\Email\Event\Trigger\Cart;
...
abstract class CartBasedSourceAbstract implements SourceInterface, ContextProviderInterface
{
    ...
    /** @var \Bronto\M2\Order\SettingsInterface  */
    protected $orderHelper;
    
    /** @var \Bronto\M2\Cart\SettingsInterface  */
    protected $cartHelper;
    
    /** @var \Bronto\M2\Core\Cookie\ReaderInterface  */
    protected $_cookie;

    /**
     * @param \Bronto\M2\Connector\SettingsInterface $connector
     * @param \Bronto\M2\Order\SettingsInterface $orderHelper
     * @param \Bronto\M2\Core\Cookie\ReaderInterface $cookie
     */
    public function __construct(
        \Bronto\M2\Connector\SettingsInterface $connector,
        \Bronto\M2\Order\SettingsInterface $orderHelper,
        \Bronto\M2\Cart\SettingsInterface $cartHelper,
        \Bronto\M2\Core\Cookie\ReaderInterface $cookie
    ) {
    ...
    protected function getTid(ExtensibleDataInterface $order)
    {
        ...
        if ($extensionAttributes) {
            ...
            $tid = $extensionAttributes->getBrontoTid();
            if ($tid) {
                return $tid->getValue();
            }
        }
